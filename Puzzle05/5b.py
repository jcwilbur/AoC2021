...
def createPlane(inputVectors):
    ...
    for vector in inputVectors:
        ...
        #diagonal
            ...
            endX = vector[1][0]
            endY = vector[1][1]
            #can loop based of x only, because assume 45deg
            xGoesBackwards = startX > endX
            xMultiplier = 1
            if xGoesBackwards: xMultiplier = -1
            yGoesBackwards = startY > endY
            yMultiplier = 1
            if yGoesBackwards: yMultiplier = -1

            for i in range (abs(startX - endX)+1):
                xval = startX + (i*xMultiplier)
                yval = startY + (i*yMultiplier)
                outputPlane[xval][yval] +=1
